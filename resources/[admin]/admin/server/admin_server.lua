...
function notifyPlayerLoggedIn(player)
	outputChatBox ("#bebebe* #ffffff"..getPlayerName(player).."#bebebe pressione #ff0000P#bebebe para abrir o painel de #ff0000administração.",player,255,255,255,true)
	local unread = 0
	for _, msg in ipairs ( aReports ) do
		unread = unread + ( msg.read and 0 or 1 )
	end
	if unread > 0 then
		outputChatBox( unread .. " mensagem de suporte não lidas" .. ( unread==1 and "" or "s" ), player, 255, 0, 0 )
	end
end
...
addCommandHandler ( "register", function ( player, command, arg1, arg2 )
	...
	if ( password ~= nil ) then
		if ( string.len ( password ) < 4 ) then
			outputChatBox ( "*  #00ff00Registro de conta#ffffff: - A senha deve ter pelo menos 4 caracteres.", player, 255, 255, 255 ,true)
		elseif ( addAccount ( username, password ) ) then
			outputChatBox ( "* #00ff00Registro de conta#ffffff: Registrado com sucesso! Usuario: '#00ff00"..username.."#ffffff', senha: '#00ff00"..password.."#ffffff'.", player, 255, 255, 255 ,true)
		...
		elseif ( getAccount ( username ) ) then
			outputChatBox ( "* #00ff00Registro de conta#ffffff: - Conta com este nome ja existe.", player, 255, 255, 255 ,true)
		else
			outputChatBox ( "* #00ff00Registro de conta#ffffff: Erro desconhecido, avise um administrador.", player, 255, 255, 255 ,true)
		end
	else
		outputChatBox ( "* #00ff00Registro de conta#ffffff: - A sintaxe é '/register nick senha'", player, 255, 255, 255 ,true)
	end
end )
...
function aAction ( type, action, admin, player, data, more )
	if ( aLogMessages[type] ) then
		...
		if ( node ) then
			...
			if ( node["all"] ) then outputChatBox ( aStripString ( node["all"] ), _root, 255, 255, 255, true ) end
			if ( node["admin"] ) and ( admin ~= player ) then outputChatBox ( aStripString ( node["admin"] ), admin, 255, 255, 255, true ) end
			if ( node["player"] ) then outputChatBox ( aStripString ( node["player"] ), player, 255, 255, 255,true ) end
...
addEventHandler ( "aTeam", _root, function ( action, name, r, g, b )
	...
	outputChatBox ( "* Acesso negado para '"..tostring ( action ).."'", source, 255, 168, 0 )
	return false
end )
...
addEventHandler ( "aAdmin", _root, function ( action, ... )
	...
	if ( action == "password" ) then
		action = nil
		if ( not arg[1] ) then outputChatBox ( "* Erro - Senha inválida.", source, 255, 0, 0 ,true)
		elseif ( not arg[2] ) then outputChatBox ( "*Erro - Nova senha inválida.", source, 255, 0, 0 ,true)
		elseif ( not arg[3] ) then outputChatBox ( "* Erro - Confirme a senha.", source, 255, 0, 0 ,true)
		elseif ( tostring ( arg[2] ) ~= tostring ( arg[3] ) ) then outputChatBox ( "* Erro - As senhas não correspondem.", source, 255, 0, 0 ,true)
		else
		...
		if ( arg[3] ) then
			...
			if ( arg[1] == "object" ) then
				...
				if ( not aclGroupAddObject ( group, object ) ) then
					action = nil
					outputChatBox ( "* Erro ao adicionar '#00ff00"..tostring ( object ).."#ffffff' ao grupo '#00ff00"..tostring ( arg[2] ).."#ffffff'", source, 255, 255, 255 ,true)
				else
					mdata2 = "Object '"..arg[3].."'"
			...
			elseif ( arg[1] == "acl" ) then
				...
				if ( not aclGroupAddACL ( group, acl ) ) then
					action = nil
					outputChatBox ( "* Erro ao adicionar ACL '#00ff00"..tostring ( arg[3] ).."#ffffff' ao grupo '#00ff00"..tostring ( arg[2] ).."#ffffff'", source, 255, 255, 255 ,true)
				else
					mdata2 = "ACL '"..arg[3].."'"
					triggerEvent ( "aAdmin", source, "sync", "aclobjects", arg[2] )
				end
			elseif ( arg[1] == "right" ) then
				...
				if ( not aclSetRight ( acl, right, enabled ) ) then
					action = nil
					outputChatBox ( "*Erro ao adicionar permissão '#00ff00"..tostring ( arg[3] ).."#ffffff' ao grupo '#00ff00"..tostring ( arg[2] ).."#ffffff'", source, 255, 255, 255 ,true)
				else
					mdata2 = "Right '"..arg[3].."'"
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		elseif ( action == "ban" ) then
			...
			if bUseSerial and getPlayerName ( player ) then
				...
				reason = reason .. " (Admin: " .. getPlayerName ( player ):gsub( "#%x%x%x%x%x%x", "" ) .. ")"
			end
			...
			if adminAccountName and adminAccountName ~= getPlayerName( source ) then
				reason = reason .. " (por " .. adminAccountName .. ")"
			end
			if bUseSerial then
				outputChatBox ( "* Você baniu o serial: " .. getPlayerSerial( player ), source, 255, 255, 255 ,true)
				setTimer ( addBan, 100, 1, nil, nil, getPlayerSerial(player), source, reason, seconds or 0 )
			else
				outputChatBox ( "Você baniu o ip: " .. getPlayerIP( player ), source, 255, 255, 255 ,true)
		...
		elseif ( action == "setnick" )  then
			...
			if setPlayerName( player, data ) then
				outputChatBox ( "* Você alterou o nome de '"..playername.."'#ffffff para '"..data.."'#ffffff!", source, 255, 255, 255 ,true)
				outputChatBox ( "* "..getPlayerName( source ).."#ffffff' alterou seu nome para '"..data.."'#ffffff!", player, 255, 255, 255 ,true)
			else
				outputChatBox ( "* Nome inválido", source, 255, 0, 0 )
			end
		elseif ( action == "shout" ) then
			local textDisplay = textCreateDisplay ()
			local textItem = textCreateTextItem ( "(ADMIN)"..getPlayerName ( source ):gsub( "#%x%x%x%x%x%x", "" )..":\n\n"..data, 0.5, 0.35, 2, 255, 255, 255, 255, 2.5, "center", "center" )
		...
		elseif ( action == "setmoney" ) then
			...
			if ( not setPlayerMoney ( player, data ) ) then
				outputChatBox ( "Dinheiro inválido", source, 255, 0, 0 )
				action = nil
			end
...
addEventHandler ( "aResource", _root, function ( name, action )
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		local text = ""
		if ( action == "start" ) then if ( startResource ( getResourceFromName ( name ), true ) ) then text = "iniciado" end
		elseif ( action == "restart" ) then if ( restartResource ( getResourceFromName ( name ) ) ) then text = "reiniciado" end
		elseif ( action == "stop" ) then if ( stopResource ( getResourceFromName ( name ) ) ) then text = "parado" end
		elseif ( action == "delete" ) then if ( deleteResource ( getResourceFromName ( name ) ) ) then text = "deletado" end
	...
	if ( action == "new" ) then
		...
		for _, p in ipairs ( getElementsByType ( "player" ) ) do
			if ( hasObjectPermissionTo ( p, "general.adminpanel" ) ) then
				outputChatBox( "* Nova mensagem do suporte recebida de " .. aReports[id].author .. "#ffffff (" .. aReports[id].subject .. ")", p, 255, 255, 255 ,true)
			end
		end
