...
function aInputBox ( title, message, default, action, vOne, vTwo )
	if ( aInputForm == nil ) then
		...
		aInputForm		= guiCreateWindow ( x / 2 - 150, y / 2 - 64, 300, 110, "", false )
				guiSetProperty(aInputForm, "CaptionColour", "FF00FFFF") 
				  	   guiWindowSetSizable ( aInputForm, false )
		aInputLabel		= guiCreateLabel ( 20, 24, 270, 15, "", false, aInputForm )
				guiLabelSetColor ( aInputLabel, 255, 255, 0 )
		guiSetFont ( aInputLabel, "default-bold-small" )	
		...
		guiSetProperty(aInputOk, "NormalTextColour", "FF00FFBB")
		aInputCancel		= guiCreateButton ( 150, 80, 55, 17, "Cancelar", false, aInputForm )
						guiSetProperty(aInputCancel, "NormalTextColour", "FF00FFBB")
...
function aBanInputBox ( player )
	...
	if ( aBanInputForm == nil ) then
		...
		guiSetProperty(aBanInputForm, "CaptionColour", "FF00FFFF") 
		...
		guiSetFont ( aBanInputLabel, "default-bold-small" )
		guiLabelSetColor ( aBanInputLabel, 255, 255, 0 )
		y = y + 23
		...
		aBanInputRadio1Label	= guiCreateLabel ( 10, 20, 270, 15, "Ban:", false, aBanInputRadioSet1 )
						guiLabelSetColor ( aBanInputRadio1Label, 255, 255, 0 )
		guiSetFont ( aBanInputRadio1Label, "default-bold-small" )	
		...
		aBanInputRadio1B		= guiCreateRadioButton ( 50, 35, 120, 15, "Serial", false, aBanInputRadioSet1 )
				guiSetFont ( aBanInputRadio1A, "default-bold-small" )
				guiSetFont ( aBanInputRadio1B, "default-bold-small" )
		...
		local yy = 5
		aBanInputRadio2Label	= guiCreateLabel ( 10, yy, 270, 15, "Duração:", false, aBanInputRadioSet2 )
						guiLabelSetColor ( aBanInputRadio2Label, 255, 255, 0 )
		guiSetFont ( aBanInputRadio2Label, "default-bold-small" )	
		aBanInputRadio2s = {}
		for i,dur in ipairs(aBanDurations) do
			aBanInputRadio2s[i] = guiCreateRadioButton ( 70, yy, 90, 15, "-", false, aBanInputRadioSet2 )
			guiSetFont ( aBanInputRadio2s[i], "default-bold-small" )
		...
		customType      = guiCreateComboBox ( 190, 4, 65, 80, "Mins", false, aBanInputRadioSet2 )
						  ...
						  guiComboBoxAddItem(customType, "Dias")
		y = y + height + 10
			
		aBanInputOk			= guiCreateButton ( 90, y, 55, 17, "Ok", false, aBanInputForm )
						guiSetProperty(aBanInputOk, "NormalTextColour", "FF00FFBB")
		aBanInputCancel		= guiCreateButton ( 150, y, 55, 17, "Cancelar", false, aBanInputForm )
				guiSetProperty(aBanInputCancel, "NormalTextColour", "FF00FFBB")
		y = y + 30
	...
	for i,dur in ipairs(aBanDurations) do
		if dur == "Custom:" then
		guiSetText ( aBanInputRadio2s[i], "Escolher:" )
		else
		guiSetText ( aBanInputRadio2s[i], dur>0 and secondsToTimeDesc(dur) or "Permanente" )
		end
	end

	guiSetText ( aBanInputForm, "Banir Jogador: " .. string.gsub(getPlayerName(player), "#%x%x%x%x%x%x", "" ))
	guiSetText ( aBanInputLabel, "Diga o motivo do banimento:" )
	aHideFloaters()
...
function aBanInputBoxFinish ()
	...
	for i,dur in ipairs(aBanDurations) do
		if guiRadioButtonGetSelected( aBanInputRadio2s[i] ) then
			if guiGetText(aBanInputRadio2s[i]) == "Escolher:" then
				...
				elseif guiComboBoxGetItemText(customType, guiComboBoxGetSelected(customType)) == "Dias" then
	...
	if not bUseIP and not bUseSerial then
		aMessageBox ( "Erro", "Selecione o tipo!" )
		return
	end

	if seconds == false then
		aMessageBox ( "Erro", "Voce nao escolheu!" )
		return
	end
...
function aMuteInputBox ( player )
	...
	if ( aMuteInputForm == nil ) then
		...
		aMuteInputForm			= guiCreateWindow ( x / 2 - 150, y / 2 - 64, 300, 150 + #aMuteDurations * 15, "", false )
							  ...
							  guiSetProperty(aMuteInputForm, "CaptionColour", "FF00FFFF") 
		guiSetAlpha(aMuteInputForm, 1)		
		y = 24

		aMuteInputLabel			= guiCreateLabel ( 20, y, 270, 15, "", false, aMuteInputForm )
		guiSetFont ( aMuteInputLabel, "default-bold-small" )
		guiLabelSetColor ( aMuteInputLabel, 255, 255, 0 )
		...
		local yy = 5
		aMuteInputRadio2Label	= guiCreateLabel ( 10, yy, 270, 15, "Duracao:", false, aMuteInputRadioSet2 )
		guiLabelSetColor ( aMuteInputRadio2Label, 255, 255, 0 )
		guiSetFont ( aMuteInputRadio2Label, "default-bold-small" )
		aMuteInputRadio2s = {}
		for i,dur in ipairs(aMuteDurations) do
			aMuteInputRadio2s[i] = guiCreateRadioButton ( 70, yy, 120, 15, "-", false, aMuteInputRadioSet2 )
					guiSetFont ( aMuteInputRadio2s[i], "default-bold-small" )
					guiLabelSetColor ( aMuteInputRadio2s[i], 255, 255, 0 )
			yy = yy + 15
		end
		y = y + height2 + 10

		aMuteInputOk			= guiCreateButton ( 90, y, 55, 17, "Ok", false, aMuteInputForm )
				guiSetProperty(aMuteInputOk, "NormalTextColour", "FF00FFBB")
		aMuteInputCancel		= guiCreateButton ( 150, y, 55, 17, "Cancelar", false, aMuteInputForm )
				guiSetProperty(aMuteInputCancel, "NormalTextColour", "FF00FFBB")
		y = y + 30
	...
	-- update duration values in the form
	for i,dur in ipairs(aMuteDurations) do
		guiSetText ( aMuteInputRadio2s[i], dur>0 and secondsToTimeDesc(dur) or "Apenas Reconect." )
	end

	guiSetText ( aMuteInputForm, "Mutar jogador: " .. getPlayerName(player):gsub( "#%x%x%x%x%x%x", "" ) )
	guiSetText ( aMuteInputLabel, "Diga o motivo para isso" )
	aHideFloaters()
...
function aMuteInputBoxFinish ()
	...
	-- Validate settings
	if seconds == false then
		aMessageBox ( "error", "Nenhuma duração selecionada!" )
		return
	end
