...
function aViewModdetails ( player )
	...
	if ( aModdetailsForm == nil ) then
		...
		aModdetailsForm	= guiCreateWindow ( x / 2 - 250, y / 2 - 125, 350, 350, "..:: Ver Moddetails ::..", false )
		guiSetProperty(aModdetailsForm, "CaptionColour", "FF00ffff")
		...
		aModdetailsRefresh	= guiCreateButton ( 0.76, 0.78, 0.32, 0.05, "Atualizar", true, aModdetailsForm )
				guiSetProperty(aModdetailsRefresh, "NormalTextColour", "FF00FFFF")
		aModdetailsClose	= guiCreateButton ( 0.76, 0.85, 0.32, 0.05, "Fechar", true, aModdetailsForm )
				guiSetProperty(aModdetailsClose, "NormalTextColour", "FF00FFFF")
	...
	guiSetText( aModdetailsForm, "Detalhes de: '" .. tostring(getPlayerName(modDetailsPlayer)):gsub( "#%x%x%x%x%x%x", "" ) .."'" )
	aHideFloaters()
...
function aModdetailsSync ( action, list, player )
	...
	if ( action == "get" ) then
		...
		aModdetailsList	= guiCreateGridList ( 0.02, 0.09, 0.72, 0.85, true, aModdetailsForm )
					   guiGridListAddColumn( aModdetailsList, "Id", 0.25 )
					   guiGridListAddColumn( aModdetailsList, "Nome", 0.60 )
