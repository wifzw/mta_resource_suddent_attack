...
wndSkin = {
	...
	controls = {
		...
		{'btn', id='set', text='Setar', onclick=applySkin, ClickSpamProtected = true},
		{'btn', id='enableModel', text='Ativar', onclick=applySkin, ClickSpamProtected = true},
		{'btn', id='disableModel', text='Desativar', onclick=applySkin, ClickSpamProtected = true},
		{'btn', id='close', text='Fechar', closeswindow=true}
	},
	oncreate = skinInit
}
...
wndAnim = {
	...
	controls = {
		...
		{'btn', id='set', text='Animar', onclick=applyAnimation, ClickSpamProtected=true},
		{'btn', id='stop', text='Parar', onclick=stopAnimation},
		{'btn', id='close', text='Fechar', closeswindow=true}
	}
}
	...
	controls = {
		{
			...
			rows={xml='xml/weapons.xml', attrs={'id', 'name'}},
			onitemdoubleclick=function(leaf) addWeapon(leaf, 500) end,
			DoubleClickSpamProtected=true
		},
		{'br'},
		{'txt', id='amount', text='500', width=60},
		{'btn', id='add', text='Adicionar', onclick=addWeapon, ClickSpamProtected=true},
		{'btn', id='close', text='Fechar', closeswindow=true}
	}
}
...
function clothesInit()
	if getElementModel(localPlayer) ~= 0 then
		errMsg('Só aplica para personagem CJ')
...
wndClothes = {
	...
	controls = {
		...
		{'br'},
		{'btn', text='add', text='Adicionar', id='addremove', width=60, onclick=applyClothes, ClickSpamProtected=true},
		{'btn', id='close', text='Fechar', closeswindow=true}
	},
	...
	controls = {
		{
			...
			rows={xml='xml/stats.xml', attrs={'name', 'id'}},
...
function loadBookmarks ()
	bookmarks = {}
	local xml = xmlLoadFile("xml/bookmarks.xml")
	if not xml then
		xml = xmlCreateFile("xml/bookmarks.xml","catalog")
	end
...
function saveBookmarks ()
	if fileExists("xml/bookmarks.xml") then
		fileDelete("xml/bookmarks.xml")
	end
	local xml = xmlCreateFile("xml/bookmarks.xml","catalog")
...
wndCreateVehicle = {
	...
	controls = {
		...
		{'btn', id='create', text='Criar', onclick=createSelectedVehicle, ClickSpamProtected=true},
		{'btn', id='enableModel', text='Ativar', onclick=createSelectedVehicle, ClickSpamProtected=true},
		{'btn', id='disableModel', text='Desativar', onclick=createSelectedVehicle, ClickSpamProtected=true},
		{'btn', id='close', text='Fechar', closeswindow=true}
	}
}
...
function upgradesInit()
	local vehicle = getPedOccupiedVehicle(localPlayer)
	if not vehicle then
		errMsg('Insira um veículo para alterar as atualizações.')
...
function paintjobInit()
	local vehicle = getPedOccupiedVehicle(localPlayer)
	if not vehicle then
		errMsg('Você precisa estar em um carro para mudar sua pintura.')
	...
	controls = {
		{
			...
			rows={xml='xml/weather.xml', attrs={'id', 'name'}},
...
function updateGUI(updateVehicle)
	if updateVehicle then
		...
		if vehicle and isElement(vehicle) then
			...
			setControlText(wndMain, 'curvehicle', 'De apé')
		end
	end
end
...
wndMain = {
	...
	width = 295,
	controls = {
		{'img', id='kill', src='imagens/Morrer.png', width=64, height=64, onclick=killLocalPlayer},
		{'img', id='skin', src='imagens/Skins.png', width=64, height=64, window=wndSkin},
		{'img', id='anim', src='imagens/Animacao.png', width=64, height=64, window=wndAnim},
		{'img', id='weapon', src='imagens/Armas.png', width=64, height=64, window=wndWeapon},
		{'img', id='clothes', src='imagens/Roupas.png', width=64, height=64, window=wndClothes},
		{'img', id='createvehicle', src='imagens/Veiculos.png', width=64, height=64, window=wndCreateVehicle},
		{'br'},
		{'chk', id='falloff', text='Não Cair da moto', onclick=toggleFallOffBike},
		{'lbl', text='Veículo Atual:'},
		{'lbl', id='curvehicle'},
		{'br'},
		{'btn', id='repair', text='Reparar', width=64, onclick=repairVehicle},
		{'btn', id='flip', text='Virar', width=64, onclick=flipVehicle},
		{'btn', id='upgrades', text='Equipar', width=64, window=wndUpgrades},
		{'btn', id='color', text='Cores', width=64, onclick=openColorPicker},
		{'btn', id='paintjob', text='Pinturas', width=64, window=wndPaintjob},
		{'br'},
		{'chk', id='lightson', text='Farol Ligado', onclick=forceLightsOn},
		{'chk', id='lightsoff', text='Falol Desligado', onclick=forceLightsOff}
	},
