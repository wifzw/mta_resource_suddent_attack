...
function createBlur()
	...
	-- sötétítés
	dxDrawRectangle ( 0,0, sx, sy, tocolor ( 210, 210, 210, 10 ) )
end

function createMinimapBlur()
	local col = tocolor (35, 35, 35,255)
	local colForBackground = tocolor (35, 35, 35,255)
	...
	dxDrawImage(0, 0, sx, sy, current, 0,0,0, col)
	-- sötétítés
	dxDrawRectangle ( 0,0, sx, sy, tocolor ( 35, 35, 35, 50 ) )
end


function blur(durum)
	if durum == true then
		addEventHandler("onClientRender",root,createMinimapBlur)
	elseif durum == false then
		removeEventHandler("onClientRender",root,createMinimapBlur)
	end
end
