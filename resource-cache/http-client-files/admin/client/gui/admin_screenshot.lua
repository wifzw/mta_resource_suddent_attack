...
function aPlayerScreenShot (player)
	if aScreenShotForm == nil then
		...
		aScreenShotForm		= guiCreateWindow	( x / 2 - 300, y / 2 - 125, 600, 250, "..:: Gerenciamento de prints ::..", false )
				guiSetProperty(aScreenShotForm, "CaptionColour", "FF00ffff") 
		aScreenShotList		= guiCreateGridList	( 0.03, 0.08, 0.70, 0.90, true, aScreenShotForm )
		aScreenShotNew		= guiCreateButton	( 0.75, 0.08, 0.42, 0.09, "Tirar print", true, aScreenShotForm )
				guiSetProperty(aScreenShotNew, "NormalTextColour", "FF00ffff")
		aScreenShotDelete	= guiCreateButton	( 0.75, 0.18, 0.42, 0.09, "Deletar", true, aScreenShotForm )
				guiSetProperty(aScreenShotDelete, "NormalTextColour", "FF00ffff")
		aScreenShotView		= guiCreateButton	( 0.75, 0.28, 0.42, 0.09, "Ver", true, aScreenShotForm )
				guiSetProperty(aScreenShotView, "NormalTextColour", "FF00ffff")
		aScreenShotRefresh	= guiCreateButton	( 0.75, 0.38, 0.42, 0.09, "Atualizar", true, aScreenShotForm )
				guiSetProperty(aScreenShotRefresh, "NormalTextColour", "FF00ffff")
		aScreenShotClose	= guiCreateButton	( 0.75, 0.88, 0.42, 0.09, "Fechar", true, aScreenShotForm )
				guiSetProperty(aScreenShotClose, "NormalTextColour", "FF00ffff")
		guiGridListAddColumn(aScreenShotList,"Jogador",0.31 )
		guiGridListAddColumn(aScreenShotList,"Admin",0.31 )
		guiGridListAddColumn(aScreenShotList,"Data",0.27 )
	...
	if button == "left" then
		...
		elseif source == aScreenShotNew then
			if guiGridListGetSelectedItem(aTab1.PlayerList ) == -1 then
				aMessageBox("Erro","Selecione um Jogador!")
			else
	...
	function (action,player,data,arg1,arg2,arg3)
		if action == "new" then
			...
			elseif isElement(player) then
				title = getPlayerName(player):gsub( "#%x%x%x%x%x%x", "" )
			else
				return
			end
			local x, y = guiGetScreenSize()
			aScreenShotWindows[player] = {}
			aScreenShotWindows[player].window = guiCreateWindow((x/2)-400,(y/2)-300,800,600,"Tirando print de: "..title,false)
			guiSetProperty(aScreenShotWindows[player].window, "CaptionColour", "FF00ffff") 
			aScreenShotWindows[player].label = guiCreateLabel(0,0,1,1,"Aguarde...",true,aScreenShotWindows[player].window)
			guiLabelSetColor ( aScreenShotWindows[player].label, 255, 255, 0 )
		    guiSetFont ( aScreenShotWindows[player].label, "default-bold-small" )	
			aScreenShotWindows[player].button = guiCreateButton(0.93,0.95,0.6,0.4,"Fechar",true,aScreenShotWindows[player].window)
			guiSetProperty(aScreenShotWindows[player].button, "NormalTextColour", "FF00FFBB")
		...
		elseif action == "list" then
			...
			for i,screenshot in ipairs (data) do
				...
				guiGridListSetItemText(aScreenShotList,row,1,screenshot.player:gsub( "#%x%x%x%x%x%x", "" ),false,false)
				guiGridListSetItemText(aScreenShotList,row,2,screenshot.admin:gsub( "#%x%x%x%x%x%x", "" ),false,false)
			...
			if action == "view" then
				...
				if isElement(player) and isElement(aScreenShotList) then
					local row = guiGridListAddRow(aScreenShotList)
					guiGridListSetItemText(aScreenShotList,row,1,getPlayerName(player):gsub( "#%x%x%x%x%x%x", "" ),false,false)
					guiGridListSetItemText(aScreenShotList,row,2,arg1:gsub( "#%x%x%x%x%x%x", "" ),false,false)
			...
			elseif action == "minimized" then
				guiSetText(aScreenShotWindows[player].label,"Jogador esta minimizado, tente novamente mais tarde.")
			elseif action == "disabled" then
				guiSetText(aScreenShotWindows[player].label,"O jogador nao permite tirar print.")
			elseif action == "quit" then
				guiSetText(aScreenShotWindows[player].label,"Jogador saiu.")
			end
		end
	end
)