...
function aPlayerStats ( player )
	if ( aStatsForm == nil ) then
		...
		aStatsForm		= guiCreateWindow ( x / 2 - 230, y / 2 - 200, 460, 400, "..:: Gerenciar Status ::..", false )
		guiSetProperty(aStatsForm, "CaptionColour", "FF00ffff") 
					   guiCreateHeader ( 0.05, 0.06, 0.40, 0.05, "Habilidades de arma:", true, aStatsForm )
		...
		while weapon <= 11 do
			...
			aStatsGUI[stat]["button"] = guiCreateButton ( 0.37, 0.05 + 0.07 * weapon, 0.10, 0.06, "Ok", true, aStatsForm, "setstat" )
			guiSetProperty(aStatsGUI[stat]["button"], "NormalTextColour", "FF00FFBB")
			guiLabelSetColor ( label, 255, 255, 0 )
		    guiSetFont ( label, "default-bold-small" )
			weapon = weapon + 1
		end
					   guiCreateStaticImage ( 0.50, 0.12, 0.0025, 0.75, "client\\images\\dot.png", true, aStatsForm )
					   guiCreateHeader ( 0.60, 0.06, 0.20, 0.05, "Corpo:", true, aStatsForm )
		local bodys = { "Gordura", "Energia", "Musculos", "Vida" }
		local body = 1
		while body <= 4 do
			local stat = body + 20
			aStatsGUI[stat] = {}
			...
			guiEditSetMaxLength ( aStatsGUI[stat]["value"], 4 )
			aStatsGUI[stat]["button"] = guiCreateButton ( 0.82, 0.05 + 0.07 * body, 0.10, 0.06, "Ok", true, aStatsForm, "setstat" )
			guiSetProperty(aStatsGUI[stat]["button"], "NormalTextColour", "FF00FFBB")
			body = body + 1
		end
		...
		local label = guiCreateLabel ( 0.50, 0.61, 0.20, 0.07, "Estilo de luta:", true, aStatsForm )
		guiLabelSetHorizontalAlign ( label, "right", false )
		aStatsGUI[300] = {}
		aStatsGUI[300]["value"] = guiCreateEdit ( 0.71, 0.60, 0.11, 0.06, "4", true, aStatsForm )
		guiEditSetMaxLength ( aStatsGUI[300]["value"], 2 )
		aStatsGUI[300]["button"] = guiCreateButton ( 0.82, 0.60, 0.10, 0.06, "Ok", true, aStatsForm )
		guiSetProperty(aStatsGUI[300]["button"], "NormalTextColour", "FF00FFBB")
		
	
		guiLabelSetColor ( label, 255, 0, 0 )
		guiSetFont ( label, "default-bold-small" )
		
		klr=guiCreateLabel ( 0.55, 0.67, 0.35, 0.07, "Valores aceitos: 4-7, 15, 16", true, aStatsForm )
		guiLabelSetColor ( klr, 255, 255, 255 )
		guiSetFont ( klr, "default-bold-small" )
		
		msg=guiCreateLabel ( 0.05, 0.93, 0.60, 0.05, "* Apenas valores de 0 a 1000 são aceitos.", true, aStatsForm )					   
		guiLabelSetColor ( msg, 255, 255, 255 )
		guiSetFont ( msg, "default-bold-small" )				   
					   
		aStatsClose		= guiCreateButton ( 0.80, 0.90, 0.14, 0.09, "Fechar", true, aStatsForm )
				guiSetProperty(aStatsClose, "NormalTextColour", "FF00FFBB")
			...
			for id, element in pairs ( aStatsGUI ) do
				if ( element["button"] == source ) then
					...
					if ( ( value ) and ( value >= 0 ) and ( value <= 1000 ) ) then
						...
						aMessageBox ( "error", "Apenas valores de (0-1000)" )
					end				
				return
				end
			end
