--------------------------------------------[Пикап]
local dxMarker = createMarker(pickupD[1],pickupD[2],pickupD[3]-0.4, "cylinder", 1, 255, 255, 255, 0)
...
ButtonDuel2 = guiCreateButton(205,470,185,35,"Desistir",false,wndDuel)
    ...
    if guiGridListGetSelectedItem(gridDuel) ~= -1 then
        if guiGridListGetSelectedItem(gridDue2) ~= -1 then
            if guiGetText(editDuel) ~= "" then
                ...
                outputChatBox("#00FF00[Duelo] #FFFFFFВInsira o valor da aposta.",255,255,255,true)
            end
        else
...
local pickupD = createPickup(pickupD[1],pickupD[2],pickupD[3], 3, 1239, 50)
local blip = createBlipAttachedTo(pickupD, 25)
setBlipVisibleDistance(pickupD, 400)

addEventHandler ( "onClientPickupHit", getRootElement(), function(ply)
	if ply ~= localPlayer then return end
	if source == pickupD then
...
function showTimer()
	...
	if timeleft >= 1 then
        ...
        dxDrawText("Antes do duelo começar "..s.." с.",scx/2,100,scx/2,10,tocolor(255,255,255),2,"default-bold","center","center")
	else
