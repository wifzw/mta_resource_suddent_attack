...
function aViewMessages ( player )
	if ( aMessagesForm == nil ) then
		...
		aMessagesForm	= guiCreateWindow ( x / 2 - 250, y / 2 - 125, 620, 250, "..:: Mensagens do suporte ::..", false )
		guiSetProperty(aMessagesForm, "CaptionColour", "FF00ffff") 
		aMessagesList		= guiCreateGridList ( 0.02, 0.09, 0.82, 0.85, true, aMessagesForm )
					   guiGridListAddColumn( aMessagesList, "#", 0.08 )
					   guiGridListAddColumn( aMessagesList, "Assunto", 0.46 )
					   guiGridListAddColumn( aMessagesList, "Data", 0.23 )
					   guiGridListAddColumn( aMessagesList, "Autor", 0.19 )
		aMessagesRead	= guiCreateButton ( 0.86, 0.20, 0.12, 0.09, "Ler", true, aMessagesForm )
				guiSetProperty(aMessagesRead, "NormalTextColour", "FF00FFFF")
		aMessagesDelete	= guiCreateButton ( 0.86, 0.30, 0.12, 0.09, "Deletar", true, aMessagesForm )
				guiSetProperty(aMessagesDelete, "NormalTextColour", "FF00FFFF")
		aMessagesRefresh	= guiCreateButton ( 0.86, 0.65, 0.12, 0.09, "Atualizar", true, aMessagesForm )
				guiSetProperty(aMessagesRefresh, "NormalTextColour", "FF00FFFF")
		aMessagesClose	= guiCreateButton ( 0.86, 0.85, 0.12, 0.09, "Fechar", true, aMessagesForm )
				guiSetProperty(aMessagesClose, "NormalTextColour", "FF00FFFF")
...
function aMessagesSync ( action, data )
	if ( action == "get" ) then
		...
		for id=#data,1,-1 do
			...
			else guiGridListSetItemText ( aMessagesList, row, 2, "* NOVO "..message.subject, false, false ) end
			guiGridListSetItemText ( aMessagesList, row, 3, message.time, false, false )
			guiGridListSetItemText ( aMessagesList, row, 4, message.author:gsub( "#%x%x%x%x%x%x", "" ), false, false )
		end
	end
end
...
function aClientMessagesClick ( button )
	if ( button == "left" ) then
		...
		elseif ( source == aMessagesRead ) then
			local row = guiGridListGetSelectedItem ( aMessagesList )
			if ( row == -1 ) then aMessageBox ( "Erro", "Selecione uma mensagem!", nil )
			else
				local id = guiGridListGetItemText ( aMessagesList, row, 1 )
				aViewMessage ( tonumber ( id ) )
			end
		elseif ( source == aMessagesDelete ) then
			local row = guiGridListGetSelectedItem ( aMessagesList )
			if ( row == -1 ) then aMessageBox ( "Erro", "Selecione uma mensagem!" )
			else
				local id = guiGridListGetItemText ( aMessagesList, row, 1 )
