...
function aManageACL ()
	if ( aAclForm == nil ) then
		...
		aAclForm		= guiCreateWindow ( x / 2 - 230, y / 2 - 250, 460, 500, "..:: Gerenciamento de Account Level - ACL ::..", false )
				guiSetProperty(aAclForm, "CaptionColour", "FF00FFFF") 
		...
		aACLCreateGroup	= guiCreateButton ( 0.55, 0.05, 0.40, 0.04, "Criar grupo", true, aAclForm )
				guiSetProperty(aACLCreateGroup, "NormalTextColour", "FF00FFBB")
		aACLCreateACL		= guiCreateButton ( 0.55, 0.10, 0.40, 0.04, "Criar ACL", true, aAclForm )
				guiSetProperty(aACLCreateACL, "NormalTextColour", "FF00FFBB")
		...
		aACLDestroyGroup	= guiCreateButton ( 0.55, 0.25, 0.40, 0.04, "Excluir Grupo", true, aAclForm )
				guiSetProperty(aACLDestroyGroup, "NormalTextColour", "FF00FFBB")
		aACLDestroyACL	= guiCreateButton ( 0.55, 0.25, 0.40, 0.04, "Excluir ACL", true, aAclForm )
				guiSetProperty(aACLDestroyACL, "NormalTextColour", "FF00FFBB")
		aACLAddObject		= guiCreateButton ( 0.55, 0.30, 0.40, 0.04, "Adicionar", true, aAclForm )
				guiSetProperty(aACLAddObject, "NormalTextColour", "FF00FFBB")
		aACLRemoveObject	= guiCreateButton ( 0.55, 0.35, 0.40, 0.04, "Remover", true, aAclForm )
				guiSetProperty(aACLRemoveObject, "NormalTextColour", "FF00FFBB")
		aACLAddACL		= guiCreateButton ( 0.55, 0.40, 0.40, 0.04, "Adicionar ACL", true, aAclForm )
				guiSetProperty(aACLAddACL, "NormalTextColour", "FF00FFBB")
		aACLRemoveACL	= guiCreateButton ( 0.55, 0.45, 0.40, 0.04, "Remover ACL", true, aAclForm )
				guiSetProperty(aACLRemoveACL, "NormalTextColour", "FF00FFBB")
		...
		aACLDropList		= guiCreateGridList ( 0.55, 0.35, 0.40, 0.30, true, aAclForm )
				guiSetFont ( aACLDropList, "default-bold-small" )
					   guiGridListAddColumn( aACLDropList, "", 0.85 )
					   guiSetVisible ( aACLDropList, false )
		aACLOk		= guiCreateButton ( 0.55, 0.40, 0.19, 0.04, "Aplicar", true, aAclForm )
				guiSetProperty(aACLOk, "NormalTextColour", "FF00FFBB")
		aACLCancel		= guiCreateButton ( 0.76, 0.40, 0.19, 0.04, "Cancelar", true, aAclForm )
				guiSetProperty(aACLCancel, "NormalTextColour", "FF00FFBB")

		aACLAddRight		= guiCreateButton ( 0.55, 0.30, 0.40, 0.04, "Permissão", true, aAclForm )
				guiSetProperty(aACLAddRight, "NormalTextColour", "FF00FFBB")
		aACLExit		= guiCreateButton ( 0.75, 0.90, 0.27, 0.04, "Fechar", true, aAclForm )
				guiSetProperty(aACLExit, "NormalTextColour", "FF00FFBB")
		aclDisplayOptions ( "", "" )
	...
	elseif ( type == "aclobjects" ) then
		...
		for i, group in ipairs ( aAclData["groups"] ) do
			...
			if ( group == acltable["name"] ) then
				...
				guiGridListSetItemText ( aACLList, aAclData["objects_row"], 2, "  Objeto:", true, false )
	...
	elseif ( type == "aclrights" ) then
		...
		for i, acl in ipairs ( aAclData["acl"] ) do
			...
			if ( acl == acltable["name"] ) then
				...
				guiGridListSetItemText ( aACLList, aAclData["rights_row"], 2, "  permissoes:", true, false )
...
function aClientACLClick ( button )
	...
	if ( button == "left" ) then
		...
		elseif ( source == aACLCreateGroup ) then
			aInputBox ( "Criar um Grupo", "Digite o nome do grupo:", "", "aclCreateGroup" )
		elseif ( source == aACLCreateACL ) then
			aInputBox ( "Criar ACL", "Digite o nome da ACL:", "", "aclCreate" )
		elseif ( source == aACLAddObject ) then
			aInputBox ( "Adicionar Objeto", "Digite o nome do objeto:", "", "aclAddObject", aAclData["current"] )
		elseif ( source == aACLAddRight ) then
			aInputBox ( "Adcionar Permisao", "Digiter a permissao a ser concedida:", "", "aclAddRight", aAclData["current"] )
		elseif ( source == aACLDestroyGroup ) then
			aMessageBox ( "Perigo", "Tem certeza que deseja destruir "..aAclData["current"].." ?", "aclDestroyGroup", aAclData["current"])
		elseif ( source == aACLDestroyACL ) then
			aMessageBox ( "Perigo", "Tem certeza que deseja destruir "..aAclData["current"].." ACL?", "aclDestroy", aAclData["current"])
		...
		elseif ( source == aACLOk ) then
			...
			elseif ( action == "Adicionar ACL:" ) then
