...
function aVehicleCustomize ( player )
	if ( aVehicleForm == nil ) then
		...
		aVehicleForm		= guiCreateWindow ( x / 2 - 300, y / 2 - 150, 600, 450, "..:: Customização do veiculo ::..", false )
		guiSetProperty(aVehicleForm, "CaptionColour", "FF00ffff") 
		...
		for i = 1, 17 do
			if ( i ~= 12 ) then
			slo=	guiCreateLabel ( 0.05, 0.05 * ( c + 1 ), 0.15, 0.05, getVehicleUpgradeSlotName ( i - 1 )..":", true, aVehicleForm )
						guiSetFont ( slo, "default-bold-small" )
		guiLabelSetColor ( slo, 255, 255, 0 )
				aVehicleUpgrades[c] = {}
		...
		aVehicleUpgradeAll	= guiCreateButton ( 0.04, 0.92, 0.15, 0.05, "Atualizar todos", true, aVehicleForm )
		guiSetProperty(aVehicleUpgradeAll, "NormalTextColour", "FF00FFBB")
		aVehicleRemoveAll	= guiCreateButton ( 0.20, 0.92, 0.18, 0.05, "Remover todos", true, aVehicleForm )
		guiSetProperty(aVehicleRemoveAll, "NormalTextColour", "FF00FFBB")
		aVehicleUpgrade	= guiCreateButton ( 0.39, 0.92, 0.23, 0.05, "Apenas selecionados", true, aVehicleForm )
		guiSetProperty(aVehicleUpgrade, "NormalTextColour", "FF00FFBB")

		guiCreateStaticImage ( 0.60, 0.10, 0.002, 0.80, "client\\images\\dot.png", true, aVehicleForm )

		pjob= guiCreateLabel ( 0.63, 0.10, 0.15, 0.05, "Pintura:", true, aVehicleForm )
		guiLabelSetColor ( pjob, 255, 255, 0 )
		guiSetFont ( pjob, "default-bold-small" )	
		...
		aVehiclePaintjobSet	= guiCreateButton ( 0.90, 0.10, 0.07, 0.048, "Ok", true, aVehicleForm )
				guiSetProperty(aVehiclePaintjobSet, "NormalTextColour", "FF00FFBB")
				coor1=	   guiCreateLabel ( 0.63, 0.15, 0.15, 0.05, "Cor do veiculo:", true, aVehicleForm )
				coor2=	   guiCreateLabel ( 0.63, 0.20, 0.15, 0.05, "Cor 1:", true, aVehicleForm )
				coor3=	   guiCreateLabel ( 0.63, 0.25, 0.15, 0.05, "Cor 2:", true, aVehicleForm )
				coor4=	   guiCreateLabel ( 0.63, 0.30, 0.15, 0.05, "Cor 3:", true, aVehicleForm )
				coor5=	   guiCreateLabel ( 0.63, 0.35, 0.15, 0.05, "Cor 4:", true, aVehicleForm )
		guiLabelSetColor ( coor1, 255, 255, 255 )
		guiSetFont ( coor1, "default-bold-small" )	
		guiLabelSetColor ( coor2, 255, 255, 0 )
		guiSetFont ( coor2, "default-bold-small" )
		guiLabelSetColor ( coor3, 255, 255, 0 )
		guiSetFont ( coor3, "default-bold-small" )	
		guiLabelSetColor ( coor4, 255, 255, 0 )
		guiSetFont ( coor4, "default-bold-small" )	
		guiLabelSetColor ( coor5, 255, 255, 0 )
		guiSetFont ( coor5, "default-bold-small" )				
		...
		aVehicleColorSet	= guiCreateButton ( 0.84, 0.41, 0.14, 0.05, "Ok", true, aVehicleForm )
		guiSetProperty(aVehicleColorSet, "NormalTextColour", "FF00FFBB")
		farol=guiCreateLabel ( 0.63, 0.5, 0.15, 0.05, "Farol:", true, aVehicleForm )
				guiLabelSetColor ( farol, 255, 255, 0 )
		guiSetFont ( farol, "default-bold-small" )	
		aLightsColor = guiCreateEdit ( 0.79, 0.5, 0.13, 0.048, "#ffffff", true, aVehicleForm ) guiEditSetMaxLength ( aLightsColor, 7 )
		aLightsColorSet	= guiCreateButton ( 0.93, 0.5, 0.05, 0.05, "Ok", true, aVehicleForm )
				guiSetProperty(aLightsColorSet, "NormalTextColour", "FF00FFBB")
		aVehicleUpgradeNames = guiCreateCheckBox ( 0.63, 0.7, 0.30, 0.04, "Mostrar nomes", true, true, aVehicleForm )
		guiSetFont ( aVehicleUpgradeNames, "default-bold-small" )
		if ( aGetSetting ( "aVehicleUpgradeNames" ) ) then guiCheckBoxSetSelected ( aVehicleUpgradeNames, true ) end
		aVehicleClose		= guiCreateButton ( 0.86, 0.92, 0.19, 0.05, "Fechar", true, aVehicleForm )
		guiSetProperty(aVehicleClose, "NormalTextColour", "FF00FFBB")
		...
		aVehicleColorClose	= guiCreateButton ( 0.86, 0.86, 0.19, 0.15, "Fechar", true, aVehicleColorForm )
				guiSetProperty(aVehicleColorClose, "NormalTextColour", "FF00FFBB")
...
function aClientVehicleClick ( button )
	...
	if ( button == "left" ) then
		...
		elseif source == aLightsColor then
			openPicker("lightsColor", "#ffffff", "Farol Cor")
