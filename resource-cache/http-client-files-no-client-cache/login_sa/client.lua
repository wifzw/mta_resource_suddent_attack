...
addEventHandler('ShowMessage', root, function(tipo, mensagem)
    outputChatBox(tipo)
    if(tipo == 'success') then
        return addNotification(mensagem,1)
    else
        return addNotification(mensagem,2)
    end
end)
...
local muzik = playSound("dosyalar/sesler/music.mp3", false)
...
-- KARAKTER SEÇME OLAY --
local karakterler = { 40, 44, 46, 69, 29, 14 } -- karaktelrini yaz
...
function log_tasarim()
    if panel_durum == "giris_ekrani" then
        ...
        button_olustur("Entrar", px, acilis_anim1, g, u, 32, 63, 104, fonte2)
        button_olustur("Registrar", px, acilis_anim1 + 30, 330, u, 160, 163, 104, fonte2)
    ...
    elseif panel_durum == "giris_bolumu" then
        ...
        editbox_olustur(guiGetText(gui_olustur[1]) .. sembol1, acilis_anim5, py + 50, g, u, tocolor(22, 81, 140, 255), "Usuário: " .. guiGetText(gui_olustur[1]), "")
        ...
        editbox_olustur(gizli_karakter_donustur(guiGetText(gui_olustur[2])) .. sembol2, acilis_anim5, py + 105, g, u, tocolor(22, 81, 140, 255), "Senha: " .. guiGetText(gui_olustur[2]), "")

        -- HESABIMI KAYDET
        checkbox_olustur("Lembrar Senha?", acilis_anim5, py + 135, 20)

        if kaydet then
            dxDrawRectangle(acilis_anim5 + 5, py + 140, 10, 10, tocolor(22, 81, 140, 255))
        end

        button_olustur("Entrar no servidor", acilis_anim5, py + 160, g, u, 244, 66, 101, fonte2)
        button_olustur(" Voltar", acilis_anim5, py + 190, g, u, 116, 22, 142, fonte2)
    ...
    elseif panel_durum == "kayit_bolumu" then
        ...
        editbox_olustur(guiGetText(gui_olustur[1]) .. sembol1, acilis_anim10, py + 15, g, u, tocolor(22, 81, 140, 255), "Usuário: " .. guiGetText(gui_olustur[1]), "")
        ...
        editbox_olustur(gizli_karakter_donustur(guiGetText(gui_olustur[2])) .. sembol2, acilis_anim10, py + 70, g, u, tocolor(22, 81, 140, 255), "Senha: " .. gizli_karakter_donustur(guiGetText(gui_olustur[2])), "")
        ...
        editbox_olustur(gizli_karakter_donustur(guiGetText(gui_olustur[3])) .. sembol3, acilis_anim10, py + 122, g, u, tocolor(22, 81, 140, 255), "Confirme a senha: " .. gizli_karakter_donustur(guiGetText(gui_olustur[3])), "")

        -- buttonlar
        button_olustur("Registrar", acilis_anim10, py + 155, g, u, 150, 127, 12, fonte2)
        button_olustur(" Voltar", acilis_anim10, py + 185, g, u, 150, 32, 11, fonte2)
    ...
    elseif panel_durum == "karakter_olusturma" then
        ...
        alt_tablo("Use as teclas de seta para a direita e para a esquerda para selecionar os caracteres.", a, y1, b, bu1, tocolor(0, 0, 0, 180))
        panel_olustur(" Bem vindo(a).", acilis_anim9 - 380 / 1.4, py - 70, 310, 170, tocolor(22, 81, 140, 2550), " Olá: #0066ff" .. getPlayerName(localPlayer):gsub("#%x%x%x%x%x%x", "") .. "", " " .. getElementData(localPlayer, "Zaman"), " " .. sehir .. "", basilimi1, basilimi2)
        button_olustur(" Vamos começar", acilis_anim9 - 380 / 1.4, py + 70, 310, u, 41, 35, 155, fonte2)
    end
end
...
addEventHandler("onClientRender", root, function()
    if yukleme_durum == true then
        ...
        dxDrawText("Aguarde enquanto os dados do seu jogador estão sendo carregados!", resim_konumlari1[1] - 50, acilis_anim_load2 + 15, 120, 25, tocolor(255, 255, 255, math.abs(math.sin(getTickCount() / 998)) * 255), 0.8, yazi_tipi)
    end
end)
